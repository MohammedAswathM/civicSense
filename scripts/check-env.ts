...
const rows = [
  ['Gemini', env.get('GEMINI_API_KEY') && env.get('NEXT_PUBLIC_DEMO_MODE') !== 'true' ? 'LIVE' : 'FALLBACK', 'Mock responses from mockResponses.ts'],
  ['Firebase', env.get('NEXT_PUBLIC_USE_FIREBASE_EMULATOR') === 'true' ? 'FALLBACK' : 'LIVE', 'Local Emulator Suite'],
  ['Maps', env.get('NEXT_PUBLIC_GOOGLE_MAPS_KEY') ? 'LIVE' : 'FALLBACK', 'Static list/map fallback'],
  ['Phone Auth', env.get('NEXT_PUBLIC_USE_FIREBASE_EMULATOR') === 'true' ? 'FALLBACK' : 'LIVE', 'Emulator OTP'],
];
